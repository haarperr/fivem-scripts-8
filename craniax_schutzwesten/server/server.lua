...
local function Sell(source, drug)
	...
	SetTimeout(v.TimeToSell * 1000, function()
		if PlayersSelling[source] == true and Drug[source] == drug then
			...
			local sellingQuantity = xPlayer.getInventoryItem(v.ItemSelling).count
			...
			if item == 0 then
				...
				xPlayer.addInventoryItem(v.ItemSelling, 1)
				--TriggerClientEvent('esx:showNotification', source, _U('sold_one', drug))
				TriggerClientEvent('craniax_notify', source, "#E16E0F", "Information", "Du hast 1x " .. v.ItemSelling .. " erhalten")
				Sell(source, drug)
			end
