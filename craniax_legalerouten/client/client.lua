...
AddEventHandler('craniax_legal:hasEnteredMarker', function(zone)
	...
	if action[2] == 'Processing' then
		esx.TriggerServerCallback('craniax_legal:getInventoryItem', function(qtd)
...
RegisterNetEvent('craniax_legal:hasExitedMarker')
AddEventHandler('craniax_legal:hasExitedMarker', function(zone)
	CurrentAction = nil
	esx.UI.Menu.CloseAll()
	TriggerServerEvent('craniax_legal:stopHarvest')
	TriggerServerEvent('craniax_legal:stopTransform')
	TriggerServerEvent('craniax_legal:stopSell')
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not hasAlreadyEnteredMarker then
			...
			TriggerEvent('craniax_legal:hasEnteredMarker', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('craniax_legal:hasExitedMarker', lastZone)
		end
	end
end)
status = false
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) then
				...
				elseif action[2] == "Field" then
					...
					TriggerServerEvent('craniax_legal:startHarvest', action[1])
					--TaskStartScenarioInPlace(playerPed, 'world_human_gardener_plant', 0, false)
					startEmote()
				elseif action[2] == "Processing" then
					TriggerServerEvent('craniax_legal:startTransform', action[1])
					startEmote()
				elseif action[2] == "Dealer" then
					TriggerServerEvent('craniax_legal:startSell', action[1])
				else
					isInZone = false
				end
				CurrentAction = nil
