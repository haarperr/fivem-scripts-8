...
AddEventHandler("onServerResourceStart", function(resName)
	if resName == "craniax_realtime" then
		Wait(500)
		while true do
			Wait(1800)
			...
			if clientrender == false then
				...
				TriggerClientEvent("craniax_realtime:event", -1, "ChangeTime", h, m, s)
			end
		end
	end
end)
...
function SetClientRender(render)
	...
	clientrender = render
	TriggerClientEvent("craniax_realtime:event", -1, "ClientServer", clientrender)
end
