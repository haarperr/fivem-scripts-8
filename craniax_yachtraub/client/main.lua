...
RegisterNetEvent('craniax_yachtraub:currentlyRobbing')
AddEventHandler('craniax_yachtraub:currentlyRobbing', function(currentStore)
	holdingUp, store = true, currentStore
end)

RegisterNetEvent('craniax_yachtraub:killBlip')
AddEventHandler('craniax_yachtraub:killBlip', function()
	RemoveBlip(blipRobbery)
end)

RegisterNetEvent('craniax_yachtraub:setBlip')
AddEventHandler('craniax_yachtraub:setBlip', function(position)
...
RegisterNetEvent('craniax_yachtraub:tooFar')
AddEventHandler('craniax_yachtraub:tooFar', function()
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_cancelled'))
end)

RegisterNetEvent('craniax_yachtraub:robberyComplete')
AddEventHandler('craniax_yachtraub:robberyComplete', function(award)
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_complete', award))
end)

RegisterNetEvent('craniax_yachtraub:startTimer')
AddEventHandler('craniax_yachtraub:startTimer', function()
	...
	while true do
					...
					if distance < 0.5 then
						...
						if IsControlJustReleased(0, Keys['E']) then
							if IsPedArmed(PlayerPedId(), 4) then
								TriggerServerEvent('craniax_yachtraub:robberyStarted', k)
							else
		...
		if holdingUp then
			...
			if Vdist(playerPos.x, playerPos.y, playerPos.z, storePos.x, storePos.y, storePos.z) > Config.MaxDistance then
				TriggerServerEvent('craniax_yachtraub:tooFar', store)
			end
		end
	end
end)
