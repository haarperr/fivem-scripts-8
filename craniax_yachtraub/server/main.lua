...
RegisterServerEvent('craniax_yachtraub:tooFar')
AddEventHandler('craniax_yachtraub:tooFar', function(currentStore)
	...
	for i=1, #xPlayers, 1 do
		...
		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('craniax_yachtraub:killBlip', xPlayers[i])
		end
	end

	if robbers[_source] then
		TriggerClientEvent('craniax_yachtraub:tooFar', _source)
		robbers[_source] = nil
		TriggerClientEvent('esx:showNotification', _source, _U('robbery_cancelled_at', Stores[currentStore].nameOfStore))
	end
end)

RegisterServerEvent('craniax_yachtraub:robberyStarted')
AddEventHandler('craniax_yachtraub:robberyStarted', function(currentStore)
	local _source  = source
			...
			if cops >= Config.policeNumberRequired then
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'police' then
						...
						TriggerClientEvent('craniax_yachtraub:setBlip', xPlayers[i], Stores[currentStore].position)
					end
				end
				...
				TriggerClientEvent('craniax_yachtraub:currentlyRobbing', _source, currentStore)
				TriggerClientEvent('craniax_yachtraub:startTimer', _source)
				...
				SetTimeout(store.secondsRemaining * 1000, function()
					if robbers[_source] then
						rob = false
						if xPlayer then
							TriggerClientEvent('craniax_yachtraub:robberyComplete', _source, store.reward)
