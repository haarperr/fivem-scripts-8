...
RegisterNetEvent('craniax_bankraub:currentlyRobbing')
AddEventHandler('craniax_bankraub:currentlyRobbing', function(currentStore)
	holdingUp, store = true, currentStore
end)

RegisterNetEvent('craniax_bankraubs:killBlip')
AddEventHandler('craniax_bankraub:killBlip', function()
	RemoveBlip(blipRobbery)
end)

RegisterNetEvent('craniax_bankraub:setBlip')
AddEventHandler('craniax_bankraub:setBlip', function(position)
...
RegisterNetEvent('craniax_bankraub:tooFar')
AddEventHandler('craniax_bankraub:tooFar', function()
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_cancelled'))
end)

RegisterNetEvent('craniax_bankraub:robberyComplete')
AddEventHandler('craniax_bankraub:robberyComplete', function(award)
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_complete', award))
end)

RegisterNetEvent('craniax_bankraub:startTimer')
AddEventHandler('craniax_bankraub:startTimer', function()
	...
	while true do
					...
					if distance < 0.5 then
						...
						if IsControlJustReleased(0, Keys['E']) then
							if IsPedArmed(PlayerPedId(), 4) then
								TriggerServerEvent('craniax_bankraub:robberyStarted', k)
							else
		...
		if holdingUp then
			...
			if Vdist(playerPos.x, playerPos.y, playerPos.z, storePos.x, storePos.y, storePos.z) > Config.MaxDistance then
				TriggerServerEvent('craniax_bankraub:tooFar', store)
			end
		end
	end
end)
