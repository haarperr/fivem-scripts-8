...
RegisterServerEvent('craniax_bankraub:tooFar')
AddEventHandler('craniax_bankraub:tooFar', function(currentStore)
	...
	for i=1, #xPlayers, 1 do
		...
		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('craniax_bankraub:killBlip', xPlayers[i])
		end
	end

	if robbers[_source] then
		TriggerClientEvent('craniax_bankraub:tooFar', _source)
		robbers[_source] = nil
		TriggerClientEvent('esx:showNotification', _source, _U('robbery_cancelled_at', Stores[currentStore].nameOfStore))
	end
end)

RegisterServerEvent('craniax_bankraub:robberyStarted')
AddEventHandler('craniax_bankraub:robberyStarted', function(currentStore)
	local _source  = source
			...
			if cops >= Config.policeNumberRequired then
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'police' then
						...
						TriggerClientEvent('craniax_bankraub:setBlip', xPlayers[i], Stores[currentStore].position)
					end
				end
				...
				TriggerClientEvent('craniax_bankraub:currentlyRobbing', _source, currentStore)
				TriggerClientEvent('craniax_bankraub:startTimer', _source)
				...
				SetTimeout(store.secondsRemaining * 1000, function()
					if robbers[_source] then
						rob = false
						if xPlayer then
							TriggerClientEvent('craniax_bankraub:robberyComplete', _source, store.reward)
