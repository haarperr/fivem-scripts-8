...
RegisterServerEvent('craniax_humanelabs:tooFar')
AddEventHandler('craniax_humanelabs:tooFar', function(currentStore)
	...
	for i=1, #xPlayers, 1 do
		...
		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('craniax_humanelabs:killBlip', xPlayers[i])
		end
	end

	if robbers[_source] then
		TriggerClientEvent('craniax_humanelabs:tooFar', _source)
		robbers[_source] = nil
		TriggerClientEvent('esx:showNotification', _source, _U('robbery_cancelled_at', Stores[currentStore].nameOfStore))
	end
end)

RegisterServerEvent('craniax_humanelabs:robberyStarted')
AddEventHandler('craniax_humanelabs:robberyStarted', function(currentStore)
	local _source  = source
			...
			if cops >= Config.policeNumberRequired then
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'police' then
						...
						TriggerClientEvent('craniax_humanelabs:setBlip', xPlayers[i], Stores[currentStore].position)
					end
				end
				...
				TriggerClientEvent('craniax_humanelabs:currentlyRobbing', _source, currentStore)
				TriggerClientEvent('craniax_humanelabs:startTimer', _source)
				...
				SetTimeout(store.secondsRemaining * 1000, function()
					if robbers[_source] then
						rob = false
						if xPlayer then
							TriggerClientEvent('craniax_humanelabs:robberyComplete', _source, store.reward)
