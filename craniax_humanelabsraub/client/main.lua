...
RegisterNetEvent('craniax_humanelabs:currentlyRobbing')
AddEventHandler('craniax_humanelabs:currentlyRobbing', function(currentStore)
	holdingUp, store = true, currentStore
end)

RegisterNetEvent('craniax_humanelabs:killBlip')
AddEventHandler('craniax_humanelabs:killBlip', function()
	RemoveBlip(blipRobbery)
end)

RegisterNetEvent('craniax_humanelabs:setBlip')
AddEventHandler('craniax_humanelabs:setBlip', function(position)
...
RegisterNetEvent('craniax_humanelabs:tooFar')
AddEventHandler('craniax_humanelabs:tooFar', function()
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_cancelled'))
end)

RegisterNetEvent('craniax_humanelabs:robberyComplete')
AddEventHandler('craniax_humanelabs:robberyComplete', function(award)
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_complete', award))
end)

RegisterNetEvent('craniax_humanelabs:startTimer')
AddEventHandler('craniax_humanelabs:startTimer', function()
	...
	while true do
					...
					if distance < 0.5 then
						...
						if IsControlJustReleased(0, Keys['E']) then
							if IsPedArmed(PlayerPedId(), 4) then
								TriggerServerEvent('craniax_humanelabs:robberyStarted', k)
							else
		...
		if holdingUp then
			...
			if Vdist(playerPos.x, playerPos.y, playerPos.z, storePos.x, storePos.y, storePos.z) > Config.MaxDistance then
				TriggerServerEvent('craniax_humanelabs:tooFar', store)
			end
		end
	end
end)
