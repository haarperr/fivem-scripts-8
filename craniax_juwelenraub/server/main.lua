...
RegisterServerEvent('craniax_juwelenraub:tooFar')
AddEventHandler('craniax_juwelenraub:tooFar', function(currentStore)
	...
	for i=1, #xPlayers, 1 do
		...
		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('craniax_juwelenraub:killBlip', xPlayers[i])
		end
	end

	if robbers[_source] then
		TriggerClientEvent('craniax_juwelenraub:tooFar', _source)
		robbers[_source] = nil
		TriggerClientEvent('esx:showNotification', _source, _U('robbery_cancelled_at', Stores[currentStore].nameOfStore))
	end
end)

RegisterServerEvent('craniax_juwelenraub:robberyStarted')
AddEventHandler('craniax_juwelenraub:robberyStarted', function(currentStore)
	local _source  = source
			...
			if cops >= Config.policeNumberRequired then
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'police' then
						...
						TriggerClientEvent('craniax_juwelenraub:setBlip', xPlayers[i], Stores[currentStore].position)
					end
				end
				...
				TriggerClientEvent('craniax_juwelenraub:currentlyRobbing', _source, currentStore)
				TriggerClientEvent('craniax_juwelenraub:startTimer', _source)
				...
				SetTimeout(store.secondsRemaining * 1000, function()
					if robbers[_source] then
						rob = false
						if xPlayer then
							TriggerClientEvent('craniax_juwelenraub:robberyComplete', _source, store.reward)
