...
RegisterNetEvent('craniax_juwelenraub:currentlyRobbing')
AddEventHandler('craniax_juwelenraub:currentlyRobbing', function(currentStore)
	holdingUp, store = true, currentStore
end)

RegisterNetEvent('craniax_juwelenraub:killBlip')
AddEventHandler('craniax_juwelenraub:killBlip', function()
	RemoveBlip(blipRobbery)
end)

RegisterNetEvent('craniax_juwelenraub:setBlip')
AddEventHandler('craniax_juwelenraub:setBlip', function(position)
...
RegisterNetEvent('craniax_juwelenraub:tooFar')
AddEventHandler('craniax_juwelenraub:tooFar', function()
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_cancelled'))
end)

RegisterNetEvent('craniax_juwelenraub:robberyComplete')
AddEventHandler('craniax_juwelenraub:robberyComplete', function(award)
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_complete', award))
end)

RegisterNetEvent('craniax_juwelenraub:startTimer')
AddEventHandler('craniax_juwelenraub:startTimer', function()
	...
	while true do
					...
					if distance < 0.5 then
						...
						if IsControlJustReleased(0, Keys['E']) then
							if IsPedArmed(PlayerPedId(), 4) then
								TriggerServerEvent('craniax_juwelenraub:robberyStarted', k)
							else
		...
		if holdingUp then
			...
			if Vdist(playerPos.x, playerPos.y, playerPos.z, storePos.x, storePos.y, storePos.z) > Config.MaxDistance then
				TriggerServerEvent('craniax_juwelenraub:tooFar', store)
			end
		end
	end
end)
